...
function conectarWebSocket(nombre, tipoLogin) {
  ...
  socket.onmessage = (evento) => {
    ...
    if (datos.tipo === 'chat') {
      ...
      datos.mensajes.forEach(msg => {
        if (msg.tipo === 'chat') {
          mostrarMensajeChat(msg.usuario, msg.mensaje, msg.color);
        } else if (msg.tipo === 'sistema') {
          mostrarMensajeSistema(msg.mensaje);
        }
      });
    }
  ...
  socket.onerror = () => {
    mostrarMensajeSistema('❌ Error de conexión');
  };
...
function mostrarMensajeChat(usuario, mensaje, color) {
  ...
  if (esMiMensaje) {
    ...
    textoDiv.style.alignSelf = 'flex-end';
  } else {
